...
public class ModbusAgent_Panel extends JPanel {			
	...
	public ModbusAgent_Panel(){	
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getButton() == 1 && e.getClickCount() == 2) {
					// 왼쪽 버튼 더블 클릭
					// ModbusAnalyzer 인증을 완료해야 모드버스 성능 추가 기능을 사용 할 수 있다
		...
		currentState.setBounds(220, 5, 125, 45);
		...
		connectButton.setBounds(351, 11, 160, 36);
			...
			public void actionPerformed(ActionEvent e) {												
				...
				if(socket_ko != null || ClientSocket.isCurrentConnected(socket_ko)) {
					...
					if(!ClientSocket.getSimpleConnectedInfo().equalsIgnoreCase(lastConnectionInfo)) {
						componentAllClear();
						src_en.swing.ModbusAgent_Panel.componentAllClear();
					}
		...
		addModbusPerf_Button.setFont(new Font("맑은 고딕", Font.BOLD, 17));
		...
		addModbusPerf_Button.setBounds(517, 11, 80, 36);					
...
//				if(!ModbusCollectionFrame.isExist) modbusCollection = new ModbusCollectionFrame();
		...
		TX.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		...
		TXinputTextField.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		...
		// 초기화 버튼
		user_resetButton = new JButton("\uCD08\uAE30\uD654");
		...
		user_resetButton.setFont(new Font("맑은 고딕", Font.BOLD, 13));
		user_resetButton.setBounds(578, 58, 77, 31);
		user_InputPanel.add(user_resetButton);
		
		// ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
		// TX user 직접 전송 버튼
		user_sendPacketButton = new JButton("\uC804 \uC1A1");
		// 전송 버튼 클릭시 발생하는 이벤트
		user_sendPacketButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {				
				...
				if((TXinputTextField.getText() == null)|| (TXinputTextField.getText().length() == 0)) {
					Util.showMessage("<font color='red'>TX 내용 확인</font>\n입력하신 TX 내용이 없습니다" + Util.separator + "\n", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				if(isRTU) {
					...
					try {						
						...
						rx = ModbusAgent.communicate(socket_ko, tx, isRTU, ClientSocket.RESPONSE_TIMEOUT);												
		...
		user_sendPacketButton.setFont(new Font("맑은 고딕", Font.BOLD, 13));
		...
		user_expression_label.setFont(new Font("맑은 고딕", Font.BOLD, 17));
		...
		user_expression_textField.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		...
		JLabel startAddress_label = new JLabel("\uC2DC\uC791\uC8FC\uC18C");
		...
		startAddress_label.setBounds(92, 72, 77, 31);
		...
		for(int i = 0; i < 125; i++) {
			reqCount125[i] = String.valueOf(i+1) + "개";
		}
		
		String[] reqCount2000 = new String[2000];
		for(int i = 0; i < 2000; i++) {
			reqCount2000[i] = String.valueOf(i+1) + "개";
		}
		...
		requestCount_label.setBounds(465, 71, 77, 31);
		...
		// 수집 명령어 라디오 버튼
		readCommand_radioButton = new JRadioButton("\uC218\uC9D1");
		...
		readCommand_radioButton.setFont(new Font("맑은 고딕", Font.BOLD, 13));
		readCommand_radioButton.setBounds(8, 48, 61, 23);
		form_InputPanel.add(readCommand_radioButton);
		
		// 제어 명령어 라디오 버튼
		writeCommand_radioButton = new JRadioButton("\uC81C\uC5B4");
		writeCommand_radioButton.setForeground(Color.BLACK);
		writeCommand_radioButton.setFont(new Font("맑은 고딕", Font.BOLD, 13));
		...
		controlStatus_ON_Button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		controlStatus_OFF_Button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		controlValue.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		...
		form_expression_label.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		form_expression_textField.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		ActionListener commandType_radioListener = new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(type.contains("수집")) {
					...
					startAddress_label.setText("제어주소");
					requestCount_label.setText("제어  값");					
		...
		userInput_radioButton.setFont(new Font("맑은 고딕", Font.BOLD, 15));
...
//		}					
	...
	public void canNotControl() {
		...
		StringBuilder msg = new StringBuilder();
		msg.append("<font color='red'>제어 패킷 전송 불가능</font>\n");
		msg.append("의도하지 않은 제어를 방지하기 위하여 제어는 양식 입력 방식으로만 전송 가능합니다" + Util.separator + "\n");									
		Util.showMessage(msg.toString(), JOptionPane.ERROR_MESSAGE);		
	}
	...
	public boolean checkControlForm() {
		...
		try {
			if(Integer.parseInt(functionCode_comboBox.getSelectedItem().toString()) == 0x06){
				int value = Integer.parseInt(controlValue.getText());
				if((value > 32767)||(value < -32768)) {
					Util.showMessage("<p><font color='red'>제어 값 입력 오류</font>\n제어 가능 값 범위 내에서 값을 입력해주세요" + Util.separator + "\n\n제어 가능 값 : <span color='blue'>-32768 ~ 32767</span>\n" + "입력하신 제어 값 :  <span color='red'>" + value + "</span>\n", JOptionPane.ERROR_MESSAGE);
					isValid = false;
				}
			}						 
		}catch(Exception exception) {
			Util.showMessage("<font color='red'>제어 값 입력 오류</font>\n제어 값은 숫자만 입력 가능합니다" + Util.separator + "\n\n입력하신 제어 값 : <span color='red'>" + controlValue.getText() + Util.separator + "</span>\n", JOptionPane.ERROR_MESSAGE);
			isValid = false;
		}
		return isValid;
	}
	...
	public void controlSuccess() {
		StringBuilder msg = new StringBuilder();
		msg.append("<font color='blue'>제어 성공</font>\n");
		msg.append("서버로부터 정상적으로 제어 성공에 대한 응답 패킷을 수신하였습니다 " + Util.longSeparator +"\n\n");
		msg.append(String.format("제어된 주소 : 0x%04x (%s%04d)번지\n", rx.getStartAddress()&0xffff , rx.getModbusAddress(), rx.getStartAddress()+1));
		if(tx.getFunctionCode() == 0x05) {
			msg.append(String.format("제어 상태 : <span color='blue'>%s</span>\n", rx.getControlStatus()));
		}else {
			msg.append(String.format("제어 값 : <span color='blue'>%d</span>\n", rx.getControlValue()));	
		}					
	...
	public void controlFail() {
		StringBuilder msg = new StringBuilder();
		msg.append("<font color='red'>제어 실패</font>\n");
		msg.append("서버로부터 제어 성공에 대한 응답 패킷을 수신하지 못하였습니다" + Util.longSeparator + "\n");														
		Util.showMessage(msg.toString(), JOptionPane.ERROR_MESSAGE);
	}
