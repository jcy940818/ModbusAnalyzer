...
public class ProcedureGeneratorFrame extends JFrame {
	...
	public ProcedureGeneratorFrame() {
		...
		lblProcedureGenerator.setFont(new Font("맑은 고딕", Font.BOLD, 22));
		...
		JLabel lblNewLabel = new JLabel("\uD504\uB85C\uC2DC\uC800 \uC774\uB984");
		...
		sp_name.setFont(new Font("맑은 고딕", Font.PLAIN, 16));		
		...
		sp_content.setFont(new Font("맑은 고딕", Font.PLAIN, 16));
		...
		JLabel label_1 = new JLabel("\uD504\uB85C\uC2DC\uC800 \uC218\uD589 \uCFFC\uB9AC");
		label_1.setHorizontalAlignment(SwingConstants.LEFT);
		label_1.setForeground(Color.BLACK);
		label_1.setFont(new Font("맑은 고딕", Font.BOLD, 17));
		label_1.setBackground(Color.WHITE);
		label_1.setBounds(475, 77, 163, 33);
		...
		sp_param_table.setFont(new Font("맑은 고딕", Font.PLAIN, 14));
		...
		add_button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		add_button.setBackground(Color.WHITE);
		add_button.setBounds(310, 289, 67, 30);
		...
		delete_button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		delete_button.setBackground(Color.WHITE);
		delete_button.setBounds(385, 289, 67, 30);
		...
		JButton generateProcedure_button = new JButton("\uD504\uB85C\uC2DC\uC800 \uC0DD\uC131");
		generateProcedure_button.setBackground(Color.WHITE);
		generateProcedure_button.setForeground(Color.BLUE);
		generateProcedure_button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		generateProcedure_button.setBounds(867, 10, 132, 33);
		...
		generateProcedure_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JLabel insert = new JLabel(String.format("<html><span color='blue'>프로시저 생성</span>"
						+ Util.longSeparator
						+ "<br><br>생성하실 프로시저의 종류를 선택해주세요" + Util.longSeparator + "<br></html>"));
				insert.setFont(new Font("맑은 고딕", Font.BOLD, 15));
				
//				String[] categoryContent = new String[] {"공통","시설물", "RCU", "성능", "이벤트", "이벤트 내역", "통계 데이터", "프로시저 저장 경로 직접 지정"}; 
				String[] categoryContent = new String[] {"공통","시설물", "RCU", "성능", "이벤트", "이벤트 내역", "통계 데이터"};
				JComboBox catogoryComboBox = new JComboBox();				
				catogoryComboBox.setFont(new Font("맑은 고딕", Font.BOLD, 15));
...
//		tcmSchedule.getColumn(2).setCellRenderer(tScheduleCellRenderer); // 파라미터 입력 예시
	...
	public StoredProcedure createProcedure() {
		...
		// 프로시저 이름 검사
		if (name.length() < 1) {
			sb.append(Util.colorBlue("프로시저 이름"));			
			isValid = false;
			inValidCount++;
		}
		...
		if(content.length() < 1) {
			if(inValidCount > 0)
				sb.append(Util.colorBlue(", 프로시저 내용"));
			else
				sb.append(Util.colorBlue("프로시저 내용"));
		...
		if(query.length() < 1) {
			if(inValidCount > 0)
				sb.append(Util.colorBlue(", 프로시저 수행 쿼리"));
			else
				sb.append(Util.colorBlue("프로시저 수행 쿼리"));
