...
public class Multi_Analysis_Panel extends JPanel {
	...
	public Multi_Analysis_Panel(){
		...
		resetButton.setBounds(685, 50, 77, 31);
		...
		analysisButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if((TXinputTextField.getText().length()==0)&&(RXinputTextField.getText().length()==0)) {
					Util.showMessage("TX, RX 내용이 없습니다", JOptionPane.ERROR_MESSAGE);
					return;
				}else if((TXinputTextField.getText() == null)|| (TXinputTextField.getText().length() == 0)) {
					Util.showMessage("TX 내용이 없습니다", JOptionPane.ERROR_MESSAGE);
					return;
				}else if((RXinputTextField.getText() == null)|| (RXinputTextField.getText().length() == 0)) {
					Util.showMessage("RX 내용이 없습니다", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				if(isRTU) {
					...
					if(rx.isRead()) {
						...
						dataTypePanel.setVisible(false);
						Util.showMessage("TX-RX Analysis 기능에서 제어 패킷 분석은 지원하지 않습니다", JOptionPane.ERROR_MESSAGE);
					}
		...
		analysisButton.setBounds(768, 50, 71, 31);
		...
		RXinputTextField.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		...
		expression_label.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		...
		expression_textField.setFont(new Font("맑은 고딕", Font.BOLD, 13));
...
//				RXinputTextField.setText(null);
	...
	public static void resetTable(JTable table){
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"\uC21C \uC11C", "Register Address", "Modbus Address", "\uB808\uC9C0\uC2A4\uD130 \uAC12"
			}
		));
