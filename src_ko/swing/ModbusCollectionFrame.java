...
//			public void run() {
//				try {
//					ModbusCollectionFrame frame = new ModbusCollectionFrame(new AdminConsole_Info("localhost","8080","Moon","940818","Moon940818"), new ModbusFacility(1,"Modbus Facility Name",1 ,1, 1));
...
//	}
	...
	public ModbusCollectionFrame(AdminConsole_Info AdminConsole, ModbusFacility modbusFacility) {
		...
		modbusCollection.setFont(new Font("맑은 고딕", Font.BOLD, 22));
		...
		facilityName.setFont(new Font("맑은 고딕", Font.BOLD, 22));
		...
		addModbusPerf_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {		
				...
				if(isInspecting) {
					...
					sb.append(String.format("\n정렬 및 검증 스레드 수행중에는 MK119 성능 추가 기능을 사용 할 수 없습니다%s\n", Util.separator));
				...
				if(isMK119Adding) {
					...
					sb.append(String.format("이미 MK119 성능 추가 작업 스레드가 수행중입니다%s\n", Util.separator));
					sb.append(String.format("\nMK119 작업 스레드는 중복으로 실행 할 수 없습니다%s\n", Util.separator));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		useAutoEvent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(useAutoEvent.isSelected()) {
					...
					sb.append("MK119 성능 추가시 성능 이벤트가 자동으로 등록됩니다\n\n");
		...
		addButton.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(isMK119Adding) {
					StringBuilder sb = new StringBuilder();
					sb.append("<font color='red'>MK119 Adding Thread is Working</font>\n");
					sb.append(String.format("현재 MK119 성능 추가 작업 스레드가 수행중입니다%s\n", Util.separator));
					sb.append(String.format("\nMK119 작업 스레드 수행중에는 레코드를 추가 할 수 없습니다%s\n", Util.separator));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(isMK119Adding) {
					...
					sb.append("<font color='red'>MK119 Adding Thread is Working</font>\n");
					sb.append(String.format("현재 MK119 성능 추가 작업 스레드가 수행중입니다%s\n", Util.separator));
					sb.append(String.format("\nMK119 작업 스레드 수행중에는 레코드를 삭제 할 수 없습니다%s\n", Util.separator));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		sortButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(isMK119Adding) {
					...
					sb.append("<font color='red'>MK119 Adding Thread is Working</font>\n");
					sb.append(String.format("현재 MK119 성능 추가 작업 스레드가 수행중입니다%s\n", Util.separator));
					sb.append(String.format("\nMK119 작업 스레드 수행중에는 테이블 정렬 및 검증 작업을 할 수 없습니다%s\n", Util.separator));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		resetButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(isMK119Adding) {
					...
					sb.append("<font color='red'>MK119 Adding Thread is Working</font>\n");
					sb.append(String.format("현재 MK119 성능 추가 작업 스레드가 수행중입니다%s\n", Util.separator));
					sb.append(String.format("\nMK119 작업 스레드 수행중에는 테이블을 초기화 할 수 없습니다%s\n", Util.separator));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
					return;
				}
