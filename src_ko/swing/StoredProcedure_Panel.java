...
public class StoredProcedure_Panel extends JPanel {
	...
	public StoredProcedure_Panel() {
		...
		procedureDiscription_textPane.setFont(new Font("맑은 고딕", Font.PLAIN, 16));
		...
		JButton xmlReload_Button = new JButton("XML 최신화");		
		xmlReload_Button.setFocusPainted(false);
		xmlReload_Button.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		xmlReload_Button.setForeground(new Color(0, 128, 0));
		xmlReload_Button.setBackground(Color.WHITE);
		xmlReload_Button.setBounds(782, 11, 129, 35);
		...
		JButton addProcedureButton = new JButton("프로시저 추가");
		addProcedureButton.setForeground(new Color(0, 128, 0));
		addProcedureButton.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		addProcedureButton.setFocusPainted(false);
		addProcedureButton.setBackground(Color.WHITE);
		addProcedureButton.setBounds(636, 11, 142, 35);
		...
		help_button.setFont(new Font("맑은 고딕", Font.BOLD, 16));
...
//		infoPanel.add(help_button);
		...
		form_queryResetButton.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		form_queryResetButton.setFocusPainted(false);
		form_queryResetButton.setBackground(Color.WHITE);
		form_queryResetButton.setBounds(915, 11, 99, 35);
...
//		ProcedureSend_radioButton.addActionListener(querySendType_radioListener);
		...
		JLabel procedureCategory_Label = new JLabel("프로시저 종류");
		procedureCategory_Label.setHorizontalAlignment(SwingConstants.RIGHT);
		procedureCategory_Label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureCategory_Label.setBounds(214, 27, 108, 31);
...
//		procedureCategory_ComboBox.setModel(new DefaultComboBoxModel(new String[] {"공통","시설물", "RCU", "성능", "이벤트", "이벤트 내역", "통계 데이터"}));
		...
		procedureCategory_ComboBox.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureCategory_ComboBox.setBackground(Color.WHITE);
		procedureCategory_ComboBox.setBounds(331, 26, 250, 32);
		...
		JButton showQueryButton = new JButton("쿼리 확인");
		showQueryButton.setForeground(Color.BLACK);
		showQueryButton.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		openFilebutton.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		JButton procedureExcute_Button = new JButton("실 행");
		procedureExcute_Button.setFocusPainted(false);
		procedureExcute_Button.setForeground(Color.BLUE);
		procedureExcute_Button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		JButton procedureDeleteButton = new JButton("삭 제");
		procedureDeleteButton.setFocusPainted(false);
		procedureDeleteButton.setForeground(Color.RED);
		procedureDeleteButton.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		procedureDeleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				msg.append("선택하신 프로시저를 삭제 하시겠습니까?");				
				msg.append(String.format("%s%s", Util.separator, Util.separator));
				msg.append("\n\n");
				
				msg.append("프로시저 인덱스 : " + Util.colorBlue(String.valueOf(selectedProcedure.getIndex()+1)));
				msg.append(String.format("%s%s", Util.separator, Util.separator));
				msg.append("\n");
				
				msg.append("프로시저 이름 : " + Util.colorBlue(selectedProcedure.getName()));
				msg.append(String.format("%s%s", Util.separator, Util.separator));
				msg.append("\n");
				...
				if(userOption == JOptionPane.YES_OPTION) {
					...
					if(!procedureFile.exists()) {
						...
						msg.append("아래의 경로에서 선택하신 프로시저 파일을 찾을 수 없습니다");
						msg.append(String.format("%s%s", Util.separator, Util.separator));
						msg.append("\n\n");
						
						msg.append("프로시저 이름 : " + Util.colorRed(selectedProcedure.getName()));
						msg.append(String.format("%s%s", Util.separator, Util.separator));
						msg.append("\n\n");						
						msg.append("파일 경로 : " + Util.colorRed(procedureFile.getParent()));
						msg.append(String.format("%s%s", Util.separator, Util.separator));
						msg.append("\n");
						msg.append("파일 이름 : " + Util.colorRed(procedureFile.getName()));
						msg.append(String.format("%s%s", Util.separator, Util.separator));
						msg.append("\n");						
						...
						if(!procedureFile.exists()) {
							...
							msg.append("성공적으로 아래의 경로에서 프로시저 파일을 삭제하였습니다");
							msg.append(String.format("%s%s", Util.separator, Util.separator));
							msg.append("\n\n");
							...
							msg.append(String.format("%s%s", Util.separator, Util.separator));
							msg.append("\n\n");
							msg.append("파일 경로 : " + Util.colorBlue(procedureFile.getParent()).replace("\\", Util.colorRed("\\")));
							msg.append(String.format("%s%s", Util.separator, Util.separator));
							msg.append("\n");
							msg.append("파일 이름 : " + Util.colorBlue(procedureFile.getName()));
							msg.append(String.format("%s%s", Util.separator, Util.separator));
							msg.append("\n");
							...
							msg.append("아래의 경로에서 프로시저 파일 삭제를 시도하였으나 실패하였습니다");
							msg.append(String.format("%s%s", Util.separator, Util.separator));
							msg.append("\n\n");
		...
		JLabel procedureSearch_label = new JLabel("프로시저 검색");
		procedureSearch_label.setHorizontalAlignment(SwingConstants.RIGHT);
		procedureSearch_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureSearch_label.setBounds(214, 72, 108, 31);
		...
		selectedIndex_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		...
		selectedSP_label.setFont(new Font("맑은 고딕", Font.BOLD, 25));
	...
	public static void updateTable(JTable table, ArrayList<StoredProcedure> procedureList) {
		...
		table.setModel(new DefaultTableModel(content, new String[] { "순 서", "Stored Procedure" }) {
			// 테이블 셀 내용 수정 금지
	...
	public void selectProcedure(StoredProcedure sp) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(String.format("프로시저 파일 : %s%s", sp.getFileName(), System.lineSeparator()));
		sb.append(String.format("프로시저 이름 : %s%s%s", sp.getName(), System.lineSeparator(), System.lineSeparator()));
		...
		if(sp.isUseParam() && paramList != null) {
			...
			for(int i = 0; i < paramList.size(); i++) {
				...
				sb.append(String.format("파라미터 %d : %s", i+1, param.getName()));
			...
			sb.append(String.format("파라미터  : %s%s", "없음", System.lineSeparator()));
		}
		...
		try {
			...
			if (searchName.length() < 1 || searchName.equalsIgnoreCase("")) {
				...
				if (result) {
					...
					// ★ 검색 문자열을 포함할 경우
					if (isSelected) {
						c.setFont(new Font("맑은 고딕", Font.BOLD, 15));
						c.setBackground(new Color(0, 120, 215));
						c.setForeground(new Color(255, 255, 255));
						return c;
					} else {
...
public class StoredProcedure_Panel extends JPanel {
	...
	public StoredProcedure_Panel() {
		...
		sqlServerInfo.setFont(new Font("맑은 고딕", Font.BOLD, 20));
		sqlServerInfo.setBackground(Color.WHITE);
		sqlServerInfo.setBounds(255, 0, 523, 55);
		infoPanel.add(sqlServerInfo);
...
//		ProcedureSend_radioButton.addActionListener(querySendType_radioListener);
		...
		JLabel procedureCategory_Label = new JLabel("프로시저 종류");
		procedureCategory_Label.setHorizontalAlignment(SwingConstants.CENTER);
		procedureCategory_Label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureCategory_Label.setBounds(221, 27, 118, 31);
		formSendQueryPanel.add(procedureCategory_Label);
		...
		procedureCategory_ComboBox.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureCategory_ComboBox.setBackground(Color.WHITE);
		procedureCategory_ComboBox.setBounds(349, 26, 205, 32);
		procedureCategory_ComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		...
		JButton procedureExcute_Button = new JButton("수 행");
		procedureExcute_Button.setForeground(Color.BLACK);
		procedureExcute_Button.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		procedureExcute_Button.setBackground(Color.WHITE);
		procedureExcute_Button.setBounds(755, 26, 77, 32);
		formSendQueryPanel.add(procedureExcute_Button);
		form_queryResetButton.setForeground(Color.BLACK);
		form_queryResetButton.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		form_queryResetButton.setBackground(Color.WHITE);
		form_queryResetButton.setBounds(755, 71, 77, 32);
		procedureExcute_Button.addActionListener(new ActionListener() {						
			public void actionPerformed(ActionEvent e) {
		...
		JLabel procedureSearch_label = new JLabel("프로시저 검색");
		procedureSearch_label.setHorizontalAlignment(SwingConstants.CENTER);
		procedureSearch_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		procedureSearch_label.setBounds(221, 72, 118, 31);
		formSendQueryPanel.add(procedureSearch_label);
		...
		procedureSearch_textField.setHorizontalAlignment(SwingConstants.LEFT);
		procedureSearch_textField.setBackground(Color.WHITE);
		procedureSearch_textField.setBounds(349, 71, 394, 32);
		procedureSearch_textField.setColumns(10);
		procedureSearch_textField.addKeyListener(new KeyAdapter() {			
			public void keyPressed(KeyEvent e) {
				setTable(table);	
