...
import src_ko.util.XmlUtil;

public class StoredProcedure implements Comparable{
	
	// 프로시저 카테고리 상수
	public static final String STORED_PROCEDURE = "storedProcedure" + "\\" + Moon.KO; // 저장 프로시저	
	...
	private static Font titlefont = new Font("맑은 고딕", Font.BOLD, 17);
	private static Font boldfont = new Font("맑은 고딕", Font.BOLD, 16);	
	...
	public static ArrayList<StoredProcedure> loadStoredProcedure(String path, String category){
		...
		if(fs == null) {
			...
			sb.append(String.format("위의 경로에서 %s%s%s\n", Util.colorBlue(category) + " 디렉토리를 찾을 수 없습니다", Util.separator, Util.separator));							

			int menu = Util.showOption(sb.toString(), new String[] { "계속 로드", "프로시저 경로 직접 지정", "취 소" }, JOptionPane.QUESTION_MESSAGE);
					
			switch (menu) {
				case -1: // 사용자가 메뉴를 선택하지 않고 대화상자를 닫았을 경우
					sb = new StringBuilder();
					sb.append(Util.colorRed("Cancel StoredProcedure Load\n"));
					sb.append(String.format("프로시저 로드 작업을 취소하였습니다%s%s\n", Util.separator, Util.separator));							
					...
					if(f.isFile()) { 
						...
						if(ext.equalsIgnoreCase("xml")) {
							...
							try {
								...
								sb.append(String.format("%s%s%s\n\n", Util.colorBlue(category) + " 디렉토리", Util.separator, Util.separator));
	...
	public static boolean inputParameter(StoredProcedure sp) {		
		...
		try {
			...
			JLabel title = new JLabel("<html>" + Util.colorBlue("프로시저 파라미터 입력") + Util.longSeparator +"<br><br></html>");			
			...
			for (int i = 0; i < paramList.size(); i++) {
				...
				if(param.getExample().length() > 0) {
					sb.append(String.format(" ( 입력 예시 : %s )%s</html>", param.getExample(), Util.longSeparator));
				}else {
			...
			if (option == JOptionPane.OK_OPTION) {
				...
				return true;
				 ...
				 Util.showMessage("<font color='red'>프로시저 실행 취소</font>\n프로시저 실행을 취소하였습니다&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n", JOptionPane.ERROR_MESSAGE);
				 return false;
			}
