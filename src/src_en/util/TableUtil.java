...
import src_en.exception.TableLoadException;
import src_en.info.ONION_Info;
import src_en.swing.MessageFrame;
import src_en.swing.ProgressFrame;
...
public class TableUtil extends Thread{
	...
	public void run() {
		try {					
			...
			Util.showMessage(String.format("<font color='red'>Cancel loading of result table</font>\n%s&nbsp;&nbsp;&nbsp;%s", e.getMessage(), System.lineSeparator()), JOptionPane.ERROR_MESSAGE);
			DbUtil.endSQL();
		}catch (Exception e) {
	...
	public void updateTable(JTable table, ResultSet rs) throws SQLException, TableLoadException {
		...
		if(DbUtil.checkMK119Db(ONION_Info.getMk119Connection())) {
			...
			if(rowCount >= limitRowCount) {
				...
				msg.append(String.format("<font color='red'>SQL Warning</font>\nThe result table has too many rows%s%s\n\n", Util.longSeparator,Util.longSeparator));
				msg.append(String.format("Result table Row Count : <font color='blue'>%d</font>\n\n", rowCount));
				msg.append("( This message is displayed when the number of rows in the result table is more than 1,000,000 )" + Util.longSeparator + "\n");								
				msg.append("( If the result table has more than 1,000,000 rows, it cannot be saved as an <font color='red'>.xlsx File</font> )\n");
				
				int menu =  Util.showOption(msg.toString(), new String[] {"Keep going","Check the top 'n' rows","Just check query" ,"Cancel"}, JOptionPane.WARNING_MESSAGE);				
				
				switch (menu) {
				case 0: // 계속 실행
					break;
				case 1: // 상위 n개 행 조회
					String newRowCountString = (String)Util.showInput(String.format("<font color='blue'>Check the top 'n' rows</font>\nPlease enter the number of rows you want to view%s%s\n", Util.longSeparator, Util.longSeparator), JOptionPane.QUESTION_MESSAGE);
					if(newRowCountString == null) throw new SQLException("Canceled entering the top n rows");
					...
					try {						
						...
						throw new SQLException("You can enter only an integer value of 1 or more in the top n rows of the table");
					}
					...
					return;
				case 2 : // 수행 쿼리 내용만 확인 
					new MessageFrame(String.format("<html><font color='blue'>%s</font> Executed query</html>", this.sqlServerInfo), queryDetail);
					return;
				case -1 : // 사용자가 메뉴를 선택하지 않고 대화상자를 나갔을 때
				case 3: // 실행 취소
					rs.close();					
					throw new SQLException("The result table loading operation has been canceled");		
				}
			}
...
//					int menu =  Util.showOption(msg.toString(), new String[] {"계속 실행", "수행 쿼리 내용만 확인" ,"실행 취소"}, JOptionPane.WARNING_MESSAGE);				
					...
					int menu =  Util.showOption(msg.toString(), new String[] {"Keep going", "Just check query" ,"Cancel"}, JOptionPane.WARNING_MESSAGE);
		...
		try {
			...
			while (rs.next()) {
				if(!p.isRunningProgress()) {
					throw new TableLoadException("Cancel the table loading operation");
				}
