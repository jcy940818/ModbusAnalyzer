...
public class SimpleValueScan_Panel extends JPanel {
	...
	public SimpleValueScan_Panel(){	
		...
		currentState.setBounds(265, 6, 193, 45);
		...
		connectButton.setBounds(467, 11, 110, 36);
		connectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {												
				...
				try {
					...
					src_ko.swing.SimpleValueScan_Panel.socket_ko = socket_en;
					...
					if( (socket_en == null || socket_en.isClosed()) && ClientSocket.getIsFirst()) {						
						...
						src_ko.swing.SimpleValueScan_Panel.IP = IP;
						src_ko.swing.SimpleValueScan_Panel.PORT = PORT;
				...
				if(socket_en != null || ClientSocket.isCurrentConnected(socket_en)) {
					...
					if(!ClientSocket.getSimpleConnectedInfo().equalsIgnoreCase(lastConnectionInfo)) {
						componentAllClear();
						src_ko.swing.SimpleValueScan_Panel.componentAllClear();
					}
		...
		Simple_label = new JLabel("Expression");
		Simple_label.setForeground(new Color(0, 128, 0));
		Simple_label.setBackground(Color.WHITE);
		Simple_label.setHorizontalAlignment(SwingConstants.CENTER);
		Simple_label.setFont(FontManager.getFont(Font.BOLD, 17));
		Simple_label.setBounds(623, 12, 101, 34);
		infoPanel.add(Simple_label);
		...
		JLabel requestCount_label = new JLabel("Scan Count");
		...
		form_sendPacketButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(SimpleValueScan_Panel.scanRunning) {
					...
					sb.append(String.format("%s is already in progress%s%s%s",Util.colorBlue("Simple Value Scan") ,Util.separator, Util.separator, "\n\n"));
				...
				if(timeout == 0) {
					...
					sb.append(String.format("Do you really want to set the timeout to infinity and start %s?%s%s%s",Util.colorBlue("Simple Value Scan") ,Util.separator, Util.separator, "\n"));
							...
							try {
								for(int i = 0; i < txList.size(); i++) {
									...
									if(!SimpleValueScan_Panel.scanRunning) {
										...
										sb.append(String.format("%s has been stopped%s%s%s",Util.colorBlue("Simple Value Scan") ,Util.separator, Util.separator, "\n"));
										Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
										return;
									}
								...
								sb.append("An unprocessable exception occurred during the " + Util.colorBlue("Simple Value Scan") + Util.separator + "\n\n");								
		...
		autoTid_CheckBox.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(SimpleValueScan_Panel.scanRunning) {
					...
					sb.append(String.format("%s You can't set the %s option while performing the function%s%s%s",Util.colorBlue("Simple Value Scan"), Util.colorBlue("Auto TID") ,Util.separator, Util.separator, "\n"));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
...
//		packetLog.setCaretPosition(pos);
	...
	public void scanStopConfirm() {
		...
		sb.append(String.format("Shall we stop %s that we are currently performing?%s%s%s",Util.colorBlue("Simple Value Scan") ,Util.separator, Util.separator, "\n"));
