...
public class ExceptionScan_Panel extends JPanel {
	...
	public ExceptionScan_Panel(){	
		...
		currentState.setBounds(218, 6, 125, 45);
		...
		connectButton.setBounds(350, 11, 160, 36);
		connectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {												
				...
				try {
					...
					src_en.swing.ExceptionScan_Panel.socket_en = socket_ko;
					...
					if( (socket_ko == null || socket_ko.isClosed()) && ClientSocket.getIsFirst()) {						
						...
						src_en.swing.ExceptionScan_Panel.IP = IP;
						src_en.swing.ExceptionScan_Panel.PORT = PORT;
				...
				if(socket_ko != null || ClientSocket.isCurrentConnected(socket_ko)) {
					...
					if(!ClientSocket.getSimpleConnectedInfo().equalsIgnoreCase(lastConnectionInfo)) {
						componentAllClear();
						src_en.swing.ExceptionScan_Panel.componentAllClear();
					}
		...
		form_sendPacketButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(ExceptionScan_Panel.scanRunning) {
					...
					sb.append(String.format("%s 작업이 이미 수행중입니다%s%s%s",Util.colorBlue("Exception Scan") ,Util.separator, Util.separator, "\n\n"));
				...
				if(timeout == 0) {
					...
					sb.append(String.format("이대로 타임아웃 설정값을 무한으로 설정하고 %s 하시겠습니까?%s%s%s",Util.colorBlue("Exception Scan") ,Util.separator, Util.separator, "\n"));
				...
				try {
							...
							try {
								for(int i = 0; i < txList.size(); i++) {
									...
									if(!ExceptionScan_Panel.scanRunning) {
										...
										sb.append(String.format("%s 작업이 중지 되었습니다%s%s%s",Util.colorBlue("Exception Scan") ,Util.separator, Util.separator, "\n"));
										Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
										return;
									}
								...
								sb.append(Util.colorBlue("Exception Scan") + " 기능 수행중 처리 할 수 없는 예외가 발생하였습니다" + Util.separator + "\n\n");
					...
					sb.append("Exception Scan 기능 수행중 처리 할 수 없는 예외가 발생하였습니다" + Util.separator + "\n\n");
		...
		autoTid_CheckBox.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(ExceptionScan_Panel.scanRunning) {
					...
					sb.append(String.format("%s 기능 수행중에는 %s 옵션을 설정 할 수 없습니다%s%s%s",Util.colorBlue("Exception Scan"), Util.colorBlue("Auto TID") ,Util.separator, Util.separator, "\n"));
					Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
...
//		packetLog.setCaretPosition(pos);
	...
	public void scanStopConfirm() {
		...
		sb.append(String.format("현재 수행중인 %s 작업을 중지할까요?%s%s%s",Util.colorBlue("Exception Scan") ,Util.separator, Util.separator, "\n"));
