...
public class ModbusXmlGeneratorFrame extends JFrame {
	...
	private static JButton convertModbusXML_Button;
...
//			public void run() {
//				try {
//					ModbusXmlGeneratorFrame frame = new ModbusXmlGeneratorFrame();
...
//	}
	 ...
	 * Create the frame.
	 */
	public ModbusXmlGeneratorFrame() {
		ModbusXmlGeneratorFrame.isExist = true;
		modbusPerfList = new ArrayList();		
		
		setBackground(Color.WHITE);
		setResizable(false);
		setTitle("ModbusAnalyzer : Modbus XML Generator");
		...
		JLabel modbusXmlGenerator = new JLabel("Modbus XML Generator");
		...
		modbusXmlGenerator.setBounds(0, 0, 333, 55);
		actualPanel.add(modbusXmlGenerator);			
		
		convertModbusXML_Button = new JButton(new Util().getMK2Resource());
		convertModbusXML_Button.setForeground(new Color(0, 128, 0));
		convertModbusXML_Button.setText("  XML \uC0DD\uC131");
		convertModbusXML_Button.setFont(FontManager.getFont(Font.BOLD, 18));
		convertModbusXML_Button.setFocusPainted(false);
		convertModbusXML_Button.setContentAreaFilled(false);
		convertModbusXML_Button.setBorder(UIManager.getBorder("Button.border"));
		convertModbusXML_Button.setBackground(Color.WHITE);
		convertModbusXML_Button.setBounds(837, 11, 202, 36);
		convertModbusXML_Button.addActionListener(new ActionListener() {
		...
		actualPanel.add(convertModbusXML_Button);
...
//		useAutoEvent.setSelected(Event.useAutoEvent);
		...
		resetButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				resetTable(table);				
				ModbusXmlGeneratorFrame.modbusPerfList.clear();
			}
		});
	...
	@Override
	public void dispose() {
		ModbusXmlGeneratorFrame.isExist = false;
		super.dispose();
	}	
	...
	public static void resetTable(JTable table){
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"\uC21C \uC11C", "\uC131\uB2A5\uBA85", "\uAE30\uB2A5\uCF54\uB4DC", "Register \uC8FC\uC18C", "Modbus \uC8FC\uC18C", "\uB370\uC774\uD130 \uD0C0\uC785", "\uC218\uC9D1\uC8FC\uAE30", "\uB2E8 \uC704", "\uBCF4\uC815\uC2DD", "\uB9C8\uC9C0\uB9C9 \uC218\uC9D1 \uAC12", "\uC774\uC9C4 \uC0C1\uD0DC : 0", "\uC774\uC9C4 \uC0C1\uD0DC : 1", "\uB2E4\uC911 \uC0C1\uD0DC"
			}
		) {
		...
		table.getColumnModel().getColumn(3).setPreferredWidth(130); // 레지스터 주소
		table.getColumnModel().getColumn(4).setPreferredWidth(130); // 모드버스 주소
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					if(userOption != JOptionPane.YES_OPTION) {															
						...
						Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
						ModbusXmlGeneratorFrame.isConverting = false;
						return;
					}else {
		...
		for(int i = 0; i < model.getRowCount(); i++) {
			...
			*/
			...
			if(!DataType.typeMap.containsKey(dataType)) {
				...
				sb.append(String.format("\n<font color='blue'>Modbus XML Generator 지원 데이터 타입</font>%s\n", Util.separator));				
