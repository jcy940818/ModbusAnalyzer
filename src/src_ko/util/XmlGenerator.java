...
import src_ko.swing.StoredProcedure_Panel;
...
public class XmlGenerator {
        ...
        try {
            ...
            for(int i = 0; i < perfs.length; i++) {            	
                ...
                try {
                	switch(agentType) {                	
	                	case "common" :
	                		entry = getGeneralPerfEntry(perfs[i], useAutoEvent);
	                		break;
	...
	public static void generateProcedure(StoredProcedure sp, String savePath) {
        ...
        try {
			...
			try {
                ...
                StringBuilder sb = new StringBuilder();
    			sb.append("<font color='red'>Failed to XML Converting</font>\n");
    			sb.append(String.format("프로시저 생성 작업중 예외가 발생하였습니다%s\n\n", Util.separator));    			
    			sb.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
    			Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
    			return;
            }
	...
	private static String getGeneralPerfEntry(Perf perf, boolean useAutoEvent){
