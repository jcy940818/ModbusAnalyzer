...
public class Condition {
	private static Font titlefont = new Font("맑은 고딕", Font.BOLD, 19);
	private static Font boldfont = new Font("맑은 고딕", Font.BOLD, 17);	
	private static Font plainfont = new Font("맑은 고딕", Font.PLAIN, 15);
	...
	public static final int OPERATOR_OR = 1; // OR 논리 연산자
	...
	public static Condition createCondition() {
		...
		try {											 			
			JLabel title = new JLabel("<html>Add a Condition for a Query<br><br></html>");
			title.setFont(titlefont);
			
			//★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
			
			// 1. 논리 연산자 선택 문자열
			String selectOperator = "<html><font color='blue'>1. Logical operator</font></html>";
...
//			operator_help.setFont(new Font("맑은 고딕", Font.BOLD, 16));
...
//			});
			...
			// 2. 컬럼 선택 문자열
			String selectColumn = "<html><br><font color='blue'>2. Condition Column</font></html>";																		
...
//			});
			...
			// 3. 조건 선택 문자열
			String selectCondition = "<html><br><font color='blue'>3. Condition Content</font></html>";
			...
			String[] conditions = {
					"01. Inquire items that have the same column content as parameter content  ",
					"02. Inquire items whose column contents are not the same as the parameter contents  ",
					
					"03. Inquire items with a column value greater than or equal to a parameter value  ",
					"04. Inquire an item whose column value is less than or equal to the parameter value  ",
					
					"05. Inquire an item whose column value is greater than the parameter value  ",
					"06. Inquire an item whose column value is smaller than the parameter value  ",
					
					"07. ( Characters are not case sensitive ) Inquire items that start with parameter contents  ",
					"08. ( Characters are not case sensitive ) Inquire items that end with parameter content  ",					
					"09. ( Characters are not case sensitive ) Inquiry of items containing parameter contents  ",
					"10. ( Characters are not case sensitive ) Inquire items whose column content does not contain parameter content  ",					
					
					"11. ( Characters are case sensitive ) Inquire items that start with parameter contents  ",
					"12. ( Characters are case sensitive ) Inquire items that end with parameter content  ",					
					"13. ( Characters are case sensitive ) Inquiry of items containing parameter contents  ",
					"14. ( Characters are case sensitive ) Inquire items whose column content does not contain parameter content  "	
			};
			...
			// 4. 조건 파라미터 문자열
			String conditionParam = "<html><br><font color='blue'>4. Condition Parameter</font></html>";
			...
			Object[] message = {
					title,
					insert, operator_ComboBox, 
					//operator_help, // 1. 논리 연산자 선택 ( AND, OR )
			...
			if (option == JOptionPane.OK_OPTION) {								
				...
				if(inputParam.getText().length() < 1) {
					Util.showMessage("<font color='red'>Failed to add conditions</font>"
							+ "\n"
							+ "The parameter was not entered, so the condition was added failed"
	...
	public static Condition createFirstCondition() {
		...
		try {											 			
			JLabel title = new JLabel("<html>Add a Condition for a Query<br></html>");
			title.setFont(titlefont);			
									
			// 1. 컬럼 선택 문자열
			String selectColumn = "<html><br><font color='blue'>1. Condition Column</font></html>";																		
...
//			});
			...
			// 2. 조건 선택 문자열
			String selectCondition = "<html><br><font color='blue'>2. Condition Content</font></html>";
			...
			// 3. 조건 파라미터 문자열
			String conditionParam = "<html><br><font color='blue'>3. Condition Parameter</font></html>";
	...
	public static String getConditionContent(Condition condition) {
		// [ServerInfo] 테이블의 [nServerIndex] 열 내용에서 "123"과 일치하는 항목을 조회합니다.
		// Inquire items that match "123" in the [nServerIndex] column contents of the [ServerInfo] table.
		
		StringBuilder content = new StringBuilder();
		String units = Condition.isValue(condition.getConditionContent())?"Value":"Content";
		
		if(!condition.isFirst()) {
			// 첫번째 조건이 아니면서 AND 논리연산을 사용하는 조건 일 경우
			content.append(String.format("     ├ Logical operator : %s%s", (condition.getOperator()==OPERATOR_AND)? "AND" : "OR" ,System.lineSeparator()));	
		}
		
		content.append(String.format("     ├ Table : %s%s", condition.getColumn().getTableName(), System.lineSeparator()));
		content.append(String.format("     ├ Column : %s%s", condition.getColumn().getColumnName(), System.lineSeparator()));
...
//				content.append(String.format("'%s'과(와) 일치하는 항목 조회",condition.getParam()));
				content.append(String.format("Same %s as '%s'",units, condition.getParam()));
				break;
			case CONDITION_NOT_EQUAL:
				// !=
//				content.append(String.format("'%s'과(와) 일치하지 않는 항목 조회",condition.getParam()));
				content.append(String.format("Not same %s as '%s'",units, condition.getParam()));
				break;
			case CONDITION_MORE:
				// >=
//				content.append(String.format("'%s'보다 이상인 항목 조회",condition.getParam()));
				content.append(String.format("%s is more than '%s'",units, condition.getParam()));
				break;
			case CONDITION_LESS:
				// <=
//				content.append(String.format("'%s'보다 이하인 항목 조회",condition.getParam()));
				content.append(String.format("%s is '%s' or less",units, condition.getParam()));
				break;
			case CONDITION_EXCEED:
				// >
//				content.append(String.format("'%s'보다 초과인 항목 조회",condition.getParam()));
				content.append(String.format("%s is exceeds '%s'",units, condition.getParam()));
				break;
			case CONDITION_LESS_THAN:
				// <
//				content.append(String.format("'%s'보다 미만인 항목 조회",condition.getParam()));
				content.append(String.format("%s is less than '%s'",units, condition.getParam()));
				break;
...
//				content.append(String.format("'%s'으로 시작하는 항목 조회",condition.getParam()));
				...
				content.append(String.format("%s '%s'","Starts with", condition.getParam()));
				content.append(System.lineSeparator());
				content.append("            └ ( Characters are not case sensitive )");
				break;
			case CONDITION_END_WITH:
				// LIKE '%?'
//				content.append(String.format("%s%s             ","대소문자 구분없이", System.lineSeparator()));
//				content.append(String.format("'%s'으로 끝나는 항목 조회",condition.getParam()));
				...
				content.append(String.format("%s '%s'","Ends with", condition.getParam()));
				content.append(System.lineSeparator());
				content.append("            └ ( Characters are not case sensitive )");
				break;
			case CONDITION_INCLUDE:
...
//				content.append(String.format("'%s'을(를) 포함하는 항목 조회",condition.getParam()));
				...
				content.append(String.format("%s '%s'","Including", condition.getParam()));
				content.append(System.lineSeparator());
				content.append("            └ ( Characters are not case sensitive )");
				break;
			case CONDITION_NOT_INCLUDE:
...
//				content.append(String.format("'%s'을 포함하지 않는 항목 조회",condition.getParam()));
				...
				content.append(String.format("%s '%s'","Doesn't include", condition.getParam()));
				content.append(System.lineSeparator());
				content.append("            └ ( Characters are not case sensitive )");
				break;
...
//				content.append(String.format("'%s'으로 시작하는 항목 조회",condition.getParam()));
				...
				content.append(String.format("%s '%s'","Starts with", condition.getParam()));
				content.append(System.lineSeparator());
				content.append("            └ ( Characters are case sensitive )");
				break;
			case CONDITION_END_WITH_CASE:
				// COLLATE Korean_Wansung_CS_AS LIKE '%?'
//				content.append(String.format("%s%s             ","대소문자를 구분하여", System.lineSeparator()));
