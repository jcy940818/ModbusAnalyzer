...
public class ONION_Info {
	...
	private static String[] sqlServerInfo = null;
	...
	public static void setSqlServerInfo(String[] info) {
		ONION_Info.sqlServerInfo = new String[2];
	...
	public static String getMemberName(String enName) {
		...
		if(member.getFullName().contains("Moon") && member.getName().equals("정창용")) {
			...
			src_ko.info.ONION_Info.userName = ONION_Info.userName;
		...
		if(!member.getTeam().contains("싱가폴")) {
			ONION_Info.userFullName += "님";
			src_ko.info.ONION_Info.userFullName += "님";
		}
