...
public class TX_Info{
	...
	public static ArrayList<TX_Info> getTxList(TX_Info tx, boolean autoTidIncrement) throws IOException{
		...
		if(isRTU) {
			for(int i = 0; i < requestCount; i++) {
				item = new TX_Generator().generateReadRTU(unitId, functionCode, startAddress + i, 1);
				txList.add(item);
			}
		}else {
			...
			for(int i = 0; i < requestCount; i++) {
				...
				if(autoTidIncrement) {
					item = new TX_Generator().generateReadTCP(transactionId + i, 0x0000, 0x0006, unitId, functionCode, startAddress + i, 1);	
				}else {
					item = new TX_Generator().generateReadTCP(transactionId, 0x0000, 0x0006, unitId, functionCode, startAddress + i, 1);	
				}				
	...
	public void setFunctionCode(int functionCode) {
		...
		switch(functionCode) {
			...
			case 2 : commandType = "Register Status(ON/FF)"; functionContent="Read Input Status"; isRead=true; break;
			...
			default : commandType = "Register Value"; functionContent="Force Multiple Coils"; isRead=true; break;
		}
