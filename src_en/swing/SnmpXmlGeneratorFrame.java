...
public class SnmpXmlGeneratorFrame extends JFrame {
	
	private static final int ORDER = 0;
	private static final int PERF_NAME = 1;
	private static final int OID = 2;
	private static final int INTERVAL = 3;
	private static final int MEASURE = 4;
	private static final int SCALE_FUNCTION = 5;	
	private static final int LABLE_0 = 6;
	private static final int LABLE_1 = 7;
	private static final int LABLE_STATUS = 8;
...
//			public void run() {
//				try {
//					SnmpXmlGeneratorFrame frame = new SnmpXmlGeneratorFrame();
...
//	}
	 ...
	 * Create the frame.
	 */
	public SnmpXmlGeneratorFrame() {
		SnmpXmlGeneratorFrame.isExist = true;
		perfList = new ArrayList();		
		
		setBackground(Color.WHITE);
		setResizable(false);
		setTitle("ModbusAnalyzer : SNMP XML Generator");
		...
		JLabel snmpXmlGenerator = new JLabel("SNMP XML Generator");
		snmpXmlGenerator.setForeground(Color.BLACK);
		snmpXmlGenerator.setIcon(new Util().getSubLogoResource());
		snmpXmlGenerator.setHorizontalAlignment(SwingConstants.LEFT);
		snmpXmlGenerator.setFont(new Font("맑은 고딕", Font.BOLD, 22));
		snmpXmlGenerator.setBackground(Color.WHITE);
		snmpXmlGenerator.setBounds(0, 0, 415, 55);
		actualPanel.add(snmpXmlGenerator);			
		...
		convertXML_Button.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		...
		JLabel addressInfo = new JLabel("OID \uAC04\uD3B8 \uC5C5\uB85C\uB4DC");
...
//		useAutoEvent.setSelected(Event.useAutoEvent);
		...
		inspectButton.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		...
		JButton btnNewButton = new JButton("OID \uC785\uB825");
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!ItemUploadFrame.isExist) {
					new ItemUploadFrame("SNMP OID Upload", "SNMP OID Upload", "snmp");
				}else {
					StringBuilder sb = new StringBuilder();
		...
		resetButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				resetTable(table);				
				SnmpXmlGeneratorFrame.perfList.clear();
			}
		});
	...
	@Override
	public void dispose() {
		SnmpXmlGeneratorFrame.isExist = false;
		super.dispose();
	}	
	...
	public static void resetTable(JTable table){
		...
		table.getColumnModel().getColumn(1).setPreferredWidth(320); // 성능명
		table.getColumnModel().getColumn(2).setPreferredWidth(350); // OID
		table.getColumnModel().getColumn(3).setPreferredWidth(80); // 수집주기
		table.getColumnModel().getColumn(4).setPreferredWidth(80); // 단위
		table.getColumnModel().getColumn(5).setPreferredWidth(130); // 보정식 
		table.getColumnModel().getColumn(6).setPreferredWidth(120); // 이진 상태 : 0
		table.getColumnModel().getColumn(7).setPreferredWidth(120); // 이진 상태 : 1
		table.getColumnModel().getColumn(8).setPreferredWidth(320); // 다중 상태
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					if(userOption != JOptionPane.YES_OPTION) {															
						...
						Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
						SnmpXmlGeneratorFrame.isConverting = false;
						return;
					}else {
...
//					Arrays.sort(perfs);		
					...
					XmlGenerator.generateXML(perfs, useAutoEvent.isSelected(), encoding, "snmp");
	...
	public static void addRecord(Perf... perf) {
		try {
			...
			for(int i = 0; i < perf.length; i++) {
				...
				/* column[2] */ record.add(perf[i].getOid());  // OID
				/* column[3] */ record.add(perf[i].getInterval()); // 수집주기
				/* column[4] */ record.add(perf[i].getMeasure()); // 단위
				/* column[5] */ record.add(perf[i].getScaleFunction()); // 보정식
								
				switch(perf[i].getDataFormat()) {
				case "1" : 
					/* column[6] */ record.add(perf[i].getBinaryMap().get("0")); // 이진 상태 : 0
					/* column[7] */ record.add(perf[i].getBinaryMap().get("1")); // 이진 상태 : 1
					/* column[8] */ record.add(""); // 다중 상태
					break;
				case "2" :
					String multiStatus = Perf.parseMultiStatusSring(perf[i].getMultiStatusMap());
					/* column[6] */ record.add(""); // 이진 상태 : 0
					/* column[7] */ record.add(""); // 이진 상태 : 1					
					/* column[8] */ record.add(multiStatus); // 다중 상태
					break;
				case "3" :
					/* column[6] */ record.add(""); // 이진 상태 : 0
					/* column[7] */ record.add(""); // 이진 상태 : 1					
...
//					Arrays.sort(perfs);
	...
	public Perf[] getCollectionPerfList(String option) {
		...
		for(int i = 0; i < model.getRowCount(); i++) {
			...
			// OID ---------------------------------------------------------------------------------
			String oid = String.valueOf(data.get(OID)).trim();						
			perfs[i].setOid(oid);
...
//		tcmSchedule.getColumn(2).setCellRenderer(tScheduleCellRenderer); // OID
		tcmSchedule.getColumn(3).setCellRenderer(tScheduleCellRenderer); // 수집 주기
		tcmSchedule.getColumn(4).setCellRenderer(tScheduleCellRenderer); // 단위
		tcmSchedule.getColumn(5).setCellRenderer(tScheduleCellRenderer); // 보정식
		tcmSchedule.getColumn(6).setCellRenderer(tScheduleCellRenderer); // 이진 상태 : 0
		tcmSchedule.getColumn(7).setCellRenderer(tScheduleCellRenderer); // 이진 상태 : 1
//		tcmSchedule.getColumn(8).setCellRenderer(tScheduleCellRenderer); // 다중 상태	
	}
	...
	public static void setTable(JTable table) {
		SnmpXmlGeneratorFrame.table = table;
	}
}
