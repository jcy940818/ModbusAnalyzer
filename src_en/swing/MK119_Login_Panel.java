...
public class MK119_Login_Panel extends JPanel {
	...
	public MK119_Login_Panel() {
		...
		agentType_Label.setFont(new Font("맑은 고딕", Font.BOLD, 25));		
	...
	public static void loginMK119() throws Exception {
		...
		if(agentType.equals("ModbusAgent") && AdminConsole_LoginFrame.isExist) {
			...
			sb.append("AdminConsole Login frame already exists" + Util.separator + "\n");
		...
		if ((mk119Connection == null) || (ONION_Info.getMk119Connection() == null)) {
			Util.showMessage("<font color='red'>Database connection failed</font>\nDatabase connection creation failed" + Util.separator + "\n", JOptionPane.ERROR_MESSAGE);
			return;
		}
...
//		mk119Connection.close();
	...
	public static void agent_ServerViewer() {
		...
		ServerList_Panel.loadServerInfo();
	...
	public static void mk119ConnSuccess() {
		...
		JLabel msg = new JLabel(String.format("<html>&nbsp;<font color='orange'><span text-align=center>Successfully Accessed MK119 Database%s%s</span></font><br></html>", Util.separator, Util.separator));
		...
		msg2.append(String.format("%s&nbsp;2. Database Name : <font color='blue'>%s</font>%s<br><br>", longSeparator, ONION_Info.getDataBaseName() ,longSeparator));
		msg2.append(String.format("%s&nbsp;3. MK119 Version : <font color='blue'>%s</font>%s<br>", longSeparator, DbUtil.getMK119VersionInfo() ,longSeparator));				
		msg2.append("<br></html>");		
