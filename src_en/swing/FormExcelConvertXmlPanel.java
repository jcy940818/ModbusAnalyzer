...
public class FormExcelConvertXmlPanel extends JPanel {	
	...
	public FormExcelConvertXmlPanel() { 
		...
		titleLabel.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 26));
		...
		JButton uploadButton = new JButton(" Form Upload");
		uploadButton.setIcon(new Util().getExcelImage());
		uploadButton.setForeground(Color.BLACK);
		uploadButton.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 19));
		uploadButton.setFocusPainted(false);
		uploadButton.setBackground(Color.WHITE);
		uploadButton.setBounds(797, 90, 243, 70);		
		...
		downloadFormExcel_Button = new JButton(" Form Download");
		...
		downloadFormExcel_Button.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 19));		
		downloadFormExcel_Button.setBounds(797, 12, 243, 70);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(!file.exists()) {
						...
						sb.append("Form Excel file does not exist in path below" + Util.separator + Util.separator + "\n\n");
					...
					if(downloadPath != null) {
						...
						if(downloadFile.exists()) {
							...
							sb.append("Successfully downloaded the form Excel file to the following path" + Util.separator + Util.separator + "\n\n");
					...
					sb.append("Download form file failed with an unhandled exception" + Util.separator + Util.separator + "\n\n");
		...
		typeSNMP_radioButton = new JRadioButton(" SNMP Perf");		
		typeSNMP_radioButton.setForeground(Color.LIGHT_GRAY);
		typeSNMP_radioButton.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 20));
		typeSNMP_radioButton.setFocusPainted(false);
		typeSNMP_radioButton.setBackground(Color.WHITE);
		typeSNMP_radioButton.setBounds(630, 231, 150, 30);
		...
		typeControl_radioButton = new JRadioButton(" Control");		
		typeControl_radioButton.setForeground(Color.LIGHT_GRAY);
		typeControl_radioButton.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 20));
		typeControl_radioButton.setFocusPainted(false);
		typeControl_radioButton.setBackground(Color.WHITE);
		typeControl_radioButton.setBounds(790, 231, 150, 30);
		...
		lblXmlEncoding.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 20));
		...
		eucKr_button.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 20));
		...
		utf8_button.setForeground(Color.LIGHT_GRAY);
		utf8_button.setFont(new Font("¸¼Àº °íµñ", Font.BOLD, 20));
	...
	public void convertExcelToXML(File formExcel) {
		
		if(formExcel == null || !formExcel.exists()) return;
		...
		if(type.equalsIgnoreCase("control")) {
			ArrayList<ControlAction> controlList = ExcelUtil.loadControlItem(formExcel);
			if(controlList != null && controlList.size() > 0) {
				XmlGenerator.generateControlXML(controlList, encoding);
			}
			
		}else {
			ArrayList<FmsPerfItem> perfList = ExcelUtil.loadPerfItem(formExcel, type);
