...
//	}
	...
	public SearchFrame(AdminConsole_Info AdminConsole, String SqlServerInfo) {
		...
		SearchFacility.setFont(new Font("맑은 고딕", Font.BOLD, 22));
		...
		lblDeviceName.setOpaque(true);
		lblDeviceName.setForeground(Color.BLACK);
		lblDeviceName.setHorizontalAlignment(SwingConstants.CENTER);
		lblDeviceName.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		lblDeviceName.setBackground(UIManager.getColor("Button.background"));
		lblDeviceName.setBounds(12, 84, 173, 44);
		actualPanel.add(lblDeviceName);
		
		JButton searchButton = new JButton("Check");
		searchButton.setForeground(Color.BLACK);
		searchButton.setFont(new Font("맑은 고딕", Font.BOLD, 17));
		...
		searchButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					setMessage("Facility that don't exist");
		...
		searchBar_TextField.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		searchBar_TextField.setBounds(197, 84, 326, 44);
		...
		result_Label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		result_Label.setBackground(UIManager.getColor("Button.background"));
		result_Label.setBounds(12, 155, 173, 44);
		actualPanel.add(result_Label);
		
		result_value_label = new JLabel("Result");
		...
		result_value_label.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		result_value_label.setBackground(Color.GREEN);
		result_value_label.setBounds(197, 155, 212, 44);
		actualPanel.add(result_value_label);
		
		JLabel facility_label = new JLabel("Facility Type");
		...
		facility_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		facility_label.setBackground(SystemColor.menu);
		facility_label.setBounds(12, 209, 173, 44);
		actualPanel.add(facility_label);
		
		facility_value_label = new JLabel("Facility Type");
		facility_value_label.setForeground(Color.BLACK);
		facility_value_label.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		facility_value_label.setBackground(Color.WHITE);
		facility_value_label.setBounds(197, 209, 420, 44);
		actualPanel.add(facility_value_label);
		
		JLabel method_label = new JLabel("Connection Method");
		...
		method_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		method_label.setBackground(UIManager.getColor("Button.background"));
		method_label.setBounds(12, 263, 173, 44);
		actualPanel.add(method_label);
		
		connMethod_value_label = new JLabel("Connection Method");
		connMethod_value_label.setForeground(Color.BLACK);
		connMethod_value_label.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		connMethod_value_label.setBackground(Color.WHITE);
		connMethod_value_label.setBounds(197, 263, 420, 44);
		actualPanel.add(connMethod_value_label);
		
		JLabel message_label = new JLabel("Content");
		...
		message_label.setFont(new Font("맑은 고딕", Font.BOLD, 16));
		message_label.setBackground(SystemColor.menu);
		message_label.setBounds(12, 317, 173, 44);
		actualPanel.add(message_label);
		
		message_value_label = new JLabel("Content");
		message_value_label.setForeground(Color.BLACK);
		message_value_label.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		message_value_label.setBackground(Color.WHITE);
		message_value_label.setBounds(197, 317, 420, 44);
		actualPanel.add(message_value_label);
		
		addPerfMK119 = new JButton();
		addPerfMK119.setText(" Add Performance");
		addPerfMK119.setForeground(Color.BLACK);
		addPerfMK119.setFont(new Font("맑은 고딕", Font.BOLD, 18));
		...
		if(rs != null) {
			...
			if(rowCount == 0) {
				...
				if(CONN_METHOD == ModbusAgent.CONN_METHOD_MODBUS && (COMM_PROTOCOL == ModbusAgent.MODBUS_TYPE_RTU || COMM_PROTOCOL == ModbusAgent.MODBUS_TYPE_TCP)) {
					...
					setMessage("You can add Modbus Performance to Device");
					addPerfMK119.setEnabled(true);
				}else {
					setResult(false);
					setFacilityType(String.valueOf(FACILITY_TYPE));
					setConnMethod(String.valueOf(CONN_METHOD));					
					setMessage("The Facility is not a Modbus Connection");	
					addPerfMK119.setEnabled(false);
				}				
	...
	public void activeModbusCollection() {
		...
		inspect = this.message_value_label.getText().contains("add Modbus");
		...
		if(inspect) {
			if(!ModbusCollectionFrame.isExist) {				
				...
				sb.append("ModbusCollection Frame is already open" + Util.separator + "\n");
				Util.showMessage(sb.toString(), JOptionPane.ERROR_MESSAGE);
			}
		}
