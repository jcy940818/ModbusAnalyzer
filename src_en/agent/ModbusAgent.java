...
//							ModbusAgent.responseTimeout(e);
	...
	public static void responseTimeout(Exception e) throws SocketException{
		...
		msg.append("Request packet sent normally, but failed to receive normal mode bus response from device for 5 seconds" + Util.separator + Util.separator + "\n");
		msg.append("When the timeout count reaches the threshold value, the agent has a communication error state\n\n");
		msg.append("Timeout count threshold : " + ClientSocket.getLimitTimeoutCount() + "\n");
		msg.append("Current Timeout count : " + ClientSocket.getCurrentTimeoutCount() + "\n\n");
...
//		}
		...
		StringBuilder msg = new StringBuilder();
		msg.append("<font color='red'>Response Timeout</font>\n");
		msg.append("Request packet sent normally" + Util.separator + Util.separator + "\n\n");				
		msg.append("But failed to receive normal mode bus response from device for 5 seconds" + Util.separator + Util.separator + "\n\n");
		msg.append("Current Timeout count : <font color='blue'>" + ClientSocket.getCurrentTimeoutCount() + "</font>\n\n");
		msg.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
	...
	public static void maxResponseTimeoutCount(Exception e) throws IOException{
		...
		msg.append("The communication is terminated because the timeout count has reached the threshold" + Util.separator + Util.separator + "\n\n");						
		msg.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
	...
	public static void waitingLostConnection(Exception e) throws IOException{
		...
		msg.append("While we were waiting for the response, the device disconnected the connection" + Util.separator + Util.separator + "\n\n");		
		msg.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
		Util.showMessage(msg.toString(), JOptionPane.ERROR_MESSAGE);
	}
	...
	public static void serverSocketClosed(Exception e) throws IOException {
		...
		msg.append("Because the socket of the device is closed, the existing socket connection is broken" + Util.separator + Util.separator + "\n\n");		
		msg.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
		Util.showMessage(msg.toString(), JOptionPane.ERROR_MESSAGE);
		clientSocket.close();			
	}
	...
	public static void unknownException(Exception e) throws IOException {
		...
		msg.append("The communication is terminated due to an unexpected error" + Util.separator + Util.separator + "\n\n");
		msg.append(String.format("Exception Message : %s%s\n", e.getMessage(), Util.separator));
		Util.showMessage(msg.toString(), JOptionPane.ERROR_MESSAGE);
		clientSocket.close();	
	}
